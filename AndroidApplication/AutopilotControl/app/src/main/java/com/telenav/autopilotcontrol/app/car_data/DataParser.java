...
import com.telenav.autopilotcontrol.app.ui.AutopilotBrainData;
...
public class DataParser
{
    ...
    public static AutopilotBrainData autopilotBrainData;
...
//            }
    ...
    void parseAutopilotData(String autopilotJsonString) throws JSONException, Exception {

        statusObject.setMapTimestamp(System.currentTimeMillis());
        autopilotBrainData = new AutopilotBrainData();
        Log.d("Server", "got message from tablet! parsing");

        autopilotJsonObject = new JSONObject(autopilotJsonString);


        if(autopilotJsonObject.has("distance_remaining"))
        {
            double localValue = Double.valueOf((autopilotJsonObject.get("distance_remaining")).toString());
            autopilotBrainData.setRemainingDistance(localValue);
        }
        if(autopilotJsonObject.has("isNavigationStarted"))
        {
            Log.d("Navigation Flag : ", String.valueOf(autopilotJsonObject.get("isNavigationStarted")));
            autopilotBrainData.setIsNavigationStarted(Boolean.valueOf((autopilotJsonObject.get("isNavigationStarted")).toString()));
        }


        if(autopilotJsonObject.has("speed_limit"))
        {
            autopilotBrainData.setSpeedLimit(Integer.valueOf((autopilotJsonObject.get("speed_limit")).toString()));
        }
        if(autopilotJsonObject.has("is_highway_next"))
        {
            autopilotBrainData.setIsHighwayNext(Boolean.valueOf((autopilotJsonObject.get("is_highway_next")).toString()));
        }
        if(autopilotJsonObject.has("are_we_on_highway"))
        {
            autopilotBrainData.setAreWeOnHighway(Boolean.valueOf((autopilotJsonObject.get("are_we_on_highway")).toString()));
        }
    }
}
